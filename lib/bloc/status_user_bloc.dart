...
class StatusUserLoaded extends StatusUserState {
  ...
  // update max reached
  StatusUserLoaded copyWith({List<StatusUserModel> listStatusUser, bool isMaxReached, String idUser}) {
...
class StatusUserBloc extends Bloc<StatusUserEvent, StatusUserState> {
  ...
  Stream<StatusUserState> mapEventToState(StatusUserEvent event) async* {
    ...
    if(state is StatusUserUnitialized) {
      ...
      listStatusUser = await StatusUserServices.getDataApi(statusLoaded.idUser, statusLoaded.listStatusUser.length, 10);
      yield (listStatusUser.isEmpty) ? statusLoaded.copyWith(isMaxReached: true, idUser: statusLoaded.idUser) 
      : StatusUserLoaded(listStatusUser: statusLoaded.listStatusUser + listStatusUser, isMaxReached: false, idUser: statusLoaded.idUser);
    }
  }
